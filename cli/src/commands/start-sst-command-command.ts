...
import { z } from 'zod';
import { CliStartCommandOutputSchema } from '../../../shared/src/session/wire-schemas.js';
...
export class StartSstCommandCommand extends Command {
  readonly name = 'start-command';
  readonly description = 'Start a named pane command in the session.';
  ...
  override async execute(args: readonly string[], ctx: CliContext): Promise<number> {
    ...
    if (parsed.values['help'] === true) {
      ctx.stdout.write(
        'Usage: sst-puppeteer start-command --command-name <name> [--session ID | --project DIR --stage S]\n',
      );
      return EXIT_OK;
    }
