...
import { z } from 'zod';
import { CliStopCommandOutputSchema } from '../../../shared/src/session/wire-schemas.js';
...
export class StopSstCommandCommand extends Command {
  readonly name = 'stop-command';
  readonly description = 'Stop a named pane command in the session.';
  ...
  override helpSchema(): HelpSchema {
    return {
      input: z.object({
        ...
        'command-name': z.string().describe('Name of the command to stop (required)'),
        pretty: z.boolean().optional(),
      }),
      output: CliStopCommandOutputSchema,
    };
  }
  ...
  override async execute(args: readonly string[], ctx: CliContext): Promise<number> {
    ...
    if (parsed.values['help'] === true) {
      ctx.stdout.write(
        'Usage: sst-puppeteer stop-command --command-name <name> [--session ID | --project DIR --stage S]\n',
      );
      return EXIT_OK;
    }
    ...
    try {
      const raw = await client.call('stop_command', { commandName });
      const result = CliStopCommandOutputSchema.parse(raw);
