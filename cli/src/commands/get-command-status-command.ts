...
import { z } from 'zod';
import { CliGetCommandStatusOutputSchema } from '../../../shared/src/session/wire-schemas.js';
...
export class GetCommandStatusCommand extends Command {
  readonly name = 'get-command-status';
  readonly description = 'Get the current status of a named command in the session.';
  ...
  override async execute(args: readonly string[], ctx: CliContext): Promise<number> {
    ...
    if (parsed.values['help'] === true) {
      ctx.stdout.write(
        'Usage: sst-puppeteer get-command-status --command-name <name> [--session ID | --project DIR --stage S]\n',
      );
      return EXIT_OK;
    }
