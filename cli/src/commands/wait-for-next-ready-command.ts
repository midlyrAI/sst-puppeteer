...
import { z } from 'zod';
import { CliWaitForNextReadyOutputSchema } from '../../../shared/src/session/wire-schemas.js';
...
export class WaitForNextReadyCommand extends Command {
  readonly name = 'wait-for-next-ready';
  readonly description =
    'Wait until the next ready transition (useful for detecting reloads/restarts).';
  ...
  override async execute(args: readonly string[], ctx: CliContext): Promise<number> {
    ...
    if (parsed.values['help'] === true) {
      ctx.stdout.write(
        'Usage: sst-puppeteer wait-for-next-ready [--session ID | --project DIR --stage S] [--timeout <ms>] [--command-name <name>]\n',
      );
      return EXIT_OK;
    }
    ...
    try {
      const raw = await client.call('wait_for_next_ready', {
        timeoutMs,
