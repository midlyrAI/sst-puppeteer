...
import { type ISession } from '../api/session.js';
import { type WaitOptions } from '../api/session-lifecycle.js';
import { type Unsubscribe } from '../api/observable.js';
...
import { PaneLogWatcher } from '../transport/pane-log-watcher.js';
import { type CompleteEventPayload, type SstBusEvent } from '../domain/sst-bus-event.js';
import { ServerFileWatcher } from '../transport/server-file-watcher.js';
import { PaneNavigator } from '../domain/pane-navigator.js';
import { KEY } from '../domain/keystroke-encoder.js';
import { parseSstConfig } from '../domain/sst-config-parser.js';
...
export class SSTSession implements ISession {
  ...
  private _eventStream: EventStream<SstBusEvent> | null = null;
  ...
  async start(): Promise<void> {
    ...
    // Wire PtySource for debug + early-exit ring buffer
    this._ptySource = new PtySource(adapter);
    await this._ptySource.start();
    const ptySource = this._ptySource;
    void (async () => {
      for await (const ev of ptySource) {
        this._recentStdout.push(ev.stripped);
        if (this._recentStdout.length > 20) {
          this._recentStdout.shift();
        }
      }
    })();
