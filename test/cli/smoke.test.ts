...
import { CliRunner, defaultRegistry } from '../../src/cli/index.js';

const CLI_SRC_DIR = fileURLToPath(new URL('../../src/cli', import.meta.url));
...
describe('cli layer isolation (no src/mcp imports)', () => {
  it('no src/cli/** file imports from src/mcp/**', () => {
    const offenders = collectTsFiles(CLI_SRC_DIR).filter((file) =>
      /from '.*\/mcp\//.test(readFileSync(file, 'utf-8')),
    );
